...
def pacf_comparison_plot(title, samples, ylim, max_lag, plot):
    ...
    axis.set_ylim(ylim)
    axis.plot(range(max_lag+1), pacf, label="statsmodels.tsa")
...
def pacf_regression_comparison_plot(title, samples, ylim, max_lag, plot):
    figure, axis = pyplot.subplots(figsize=(10, 7))
    pacf = arima.pacf(samples, max_lag)
    pacf_reg = pacf_regression_method(samples, max_lag)
    axis.set_title(title)
    axis.set_xlabel("Time Lag (τ)")
    axis.set_xlim([-0.5, max_lag+1])
    axis.set_ylim(ylim)
    axis.plot(range(max_lag+1), pacf, label="statsmodels.tsa")
    axis.plot(range(max_lag+1), pacf_reg, label="Regression Method", marker='o', markersize=15.0, linestyle="None", markeredgewidth=1.0)
    axis.legend(fontsize=16)
    config.save_post_asset(figure, "mean_reversion", plot)
...
max_lag = 10
ylim = [-0.1, 1.1]
pacf_comparison_plot(title, ar1, ylim, max_lag, plot_name)

# %%

title = f"AR(1) PACF Comparison: " + r"$\phi=$"+f"{numpy.array2string(φ1, precision=2, separator=',')}"
plot_name = "pacf_method_comparison_ar_1_regression_pacf"
max_lag = 10
ylim = [-0.1, 1.1]
pacf_regression_comparison_plot(title, ar1, ylim, max_lag, plot_name)
...
max_lag = 10
ylim = [-0.1, 1.1]
pacf_comparison_plot(title, ar2, ylim, max_lag, plot_name)
...
max_lag = 10
ylim = [-0.1, 1.1]
pacf_regression_comparison_plot(title, ar2, ylim, max_lag, plot_name)
...
max_lag = 10
ylim = [-0.1, 1.1]
pacf_regression_comparison_plot(title, ar3, ylim, max_lag, plot_name)
